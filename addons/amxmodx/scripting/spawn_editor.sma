...
//Menu Callbacks
	...
	menu_additem(g_MainMenuID, "Сделай текущую точку спавном","1", 0, g_cMain);
	menu_additem(g_MainMenuID, "Изменить ближайший к вам спавн","2", 0, g_cMain);
	menu_additem(g_MainMenuID, "Удалить ближайший спавн","3", 0, g_cMain);
	menu_additem(g_MainMenuID, "Обновить ближайший спавн", "4", 0, g_cMain);
...
public m_MainHandler(id, menu, item)
{
	...
	switch(iChoice)
	{
		...
		{	
			...
			client_print(id, 
				print_chat,
				"Всего спавнов: %d ^nТекущая позиция: X: %f  Y: %f  Z: %f", 
				g_TotalSpawns + 1,
				Org[0],
				Org[1],
				Org[2]);

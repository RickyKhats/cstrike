...
public plugin_init()
{
	...
	g_MainMenu = menu_create("CSDM меню", "use_csdm_menu")
	
	new callback = menu_makecallback("hook_item_display")
	menu_additem(g_MainMenu, "Включить/Выключить", "csdm_ctrl", D_ACCESS, callback)
	menu_additem(g_MainMenu, "Перезагрузить конфиг", "csdm_reload", D_ACCESS)
...
public csdm_reload(id, level, cid)
{
	...
	{
		client_print(id, print_chat, "[CSDM] Файл настроек перезагружен.")
	} else {
		client_print(id, print_chat, "[CSDM] Не удалось найти файл конфигурации.")
	}
...
public use_csdm_menu(id, menu, item)
{
	...
	{
		client_print(id, print_chat, "У вас нет прав для доступа к этому меню.")
		return PLUGIN_HANDLED
	}
...
public say_respawn(id)
{
	...
	{
		client_print(id, print_chat, "[CSDM] Данная команда не работает!")
		return PLUGIN_HANDLED
	}

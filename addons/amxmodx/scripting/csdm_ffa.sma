...
public csdm_ffa_ctrl(id, level, cid)
{
	...
	client_print(id, print_chat, "[CSDM] CSDM FFA режим изменен.")
...
public hook_item_display(player, menu, item)
{
	...
	if (equali(command, "csdm_ffa_ctrl"))
	{
		if (!csdm_get_ffa())
		{
			menu_item_setname(menu, item, "FFA режим включен")
		} else {
			menu_item_setname(menu, item, "FFA режим выключен")
		}
	}
}
...
public csdm_ffa_enable(id, level, cid)
{
	...
	csdm_set_ffa(1)
	client_print(id, print_chat, "CSDM FFA режим включен.")
...
public csdm_ffa_disable(id, level, cid)
{
	...
	csdm_set_ffa(0)
	client_print(id, print_chat, "CSDM FFA режим выключен.")

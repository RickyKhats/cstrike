...
public m_EquipHandler(id, menu, item)
{
	...
	switch(choice)
	{
		...
		case 3:
		{
			...
			client_print(id, print_chat, "[CSDM] Напишите ^"guns^" в чат для перезапуска меню.")
			equipUser(id, EQUIP_ALL)
		}
	}
...
{
	//Equip Menu
	menu_additem(g_EquipMenuID, "Новое оружие", "1", 0, -1)
	menu_additem(g_EquipMenuID, "Предыдущий выбор", "2", 0, -1)
	menu_additem(g_EquipMenuID, "Предыдущий выбор на всю игру", "3", 0, -1)
	menu_setprop(g_EquipMenuID, MPROP_EXIT, MEXIT_NEVER)
	
	//Armor Menu
	menu_additem(g_ArmorMenuID, "Да, с броней", "1", 0, -1)
	menu_additem(g_ArmorMenuID, "Без брони", "2", 0, -1)
	menu_setprop(g_ArmorMenuID, MPROP_EXIT, MEXIT_NEVER)
	
	//Nade Menu
	menu_additem(g_NadeMenuID, "С гранатами", "1", 0, -1)
...
public enableMenu(id)
{
	...
	{
		g_mShowuser[id] = true
		client_print(id, print_chat, "[CSDM] Меню экипировки включено снова.")
		if (!g_MenuState[id])
		{
			g_MenuState[id] = 1
		...
		client_print(id, print_chat, "[CSDM] Меню экипировки включено.")
	}

...
new const g_SENTRYFRAGREWARDS[3] = {500, 500, 500}		// how many $ you get if your sentry frags someone. If you built and upgraded to level 3 you would get $300 + $150 = $450. If built and upgraded all you would get $600.
...
public createsentryhere(id) {
	...
	if(!g_allowBuild)
		{
		client_print(id, print_center, "%L", LANG_SERVER, "SEN_WAIT")
	...
	if (sentry && entity_range(sentry, id) <= MAXUPGRADERANGE) {
		...
		// Don't allow builder to upgrade his own sentry first time.
			if (entity_get_int(sentry, SENTRY_INT_LEVEL) == SENTRY_LEVEL_1 && id == GetSentryPeople(sentry, OWNER)) {
				client_print(id, print_center, "%L", LANG_SERVER, "SEN_TEAM_LVL2")
				return PLUGIN_HANDLED
			}
		#endif
			...
			if (entity_get_int(sentry, SENTRY_INT_LEVEL) == SENTRY_LEVEL_2 && id == GetSentryPeople(sentry, UPGRADER_1)) {
				client_print(id, print_center, "%L", LANG_SERVER, "SEN_TEAM_LVL3")
				return PLUGIN_HANDLED
			}
		#endif
...
if (GetSentryCount(id) >= MAXPLAYERSENTRIES) {
	...
	client_print(id, print_center,  "%L", LANG_SERVER, "SEN_ONLY_BUILD", wordnumbers, maxsentries != 1 ? "s" : "")
	return
}
else if (g_inBuilding[id - 1]) {
	client_print(id, print_center, "%L", LANG_SERVER, "SEN_FAST_BUILD")
	return
}
	...
	client_print(id, print_center, "%L", LANG_SERVER, "SEN_ENOUGH_MONEY", g_COST(0))
	return
}
else if (!entity_is_on_ground(id)) {
	client_print(id, print_center, "%L", LANG_SERVER, "SEN_STAND")
	return
}
//else if (entity_get_int(id, EV_INT_flags) & FL_DUCKING) {
else if (entity_get_int(id, EV_INT_bInDuck)) {
	client_print(id, print_center, "%L", LANG_SERVER, "SEN_SITTING")
	return
}
else if ( get_pcvar_num( sentry_team ) ) {
	if( get_pcvar_num( sentry_team ) < 0 && !is_user_admin(id) ){
		client_print(id, print_center, "%L", LANG_SERVER, "SEN_ADMIN_BUILD")
		return
	}
	else if( !(abs(get_pcvar_num( sentry_team )) & _:cs_get_user_team(id)) ){
		client_print(id, print_center, "%L", LANG_SERVER, "SEN_TEAM_BUILD")
		return
	}
}
...
if (CreateSentryBase(vNewOrigin, id)) {
	...
	client_print(id, print_center, "%L", LANG_SERVER, "SEN_DONT_CREATE")
}
}
...
for (new i = 1; i <= g_MAXPLAYERS; i++) {
	if (!is_user_connected(i) || !is_user_alive(i) || cs_get_user_team(i) != builderTeam || id == i)
		continue
	client_print(i, print_center, "%L", LANG_SERVER, "SEN_UNITS", name, floatround(entity_range(i, sentryEntity)))
...
// Change classname
entity_set_string(entbase, EV_SZ_classname, CLASSNAME)
// Set model
...
// Set model
entity_set_model(ent, "models/tf2/sentry1.mdl") // later set according to level
// Set size
mins[0] = -16.0
mins[1] = -16.0
mins[2] = 0.0
...
emit_sound(ent, CHAN_AUTO, "tf2/turrset.wav", 1.0, ATTN_NORM, 0, PITCH_NORM)
...
if (!quiet) {
	...
	client_print(owner, print_center, "%L", LANG_SERVER, "SEN_DESTROY")
}
...
if (!is_valid_ent(parm[0])) {
	client_print(0, print_chat, "", parm[0])
	return
}
...
if (random_num(0, 99) < 10)
	emit_sound(ent, CHAN_AUTO, "tf2/ping.wav", 1.0, ATTN_NORM, 0, PITCH_NORM)
...
if (closestTarget) {
	...
	emit_sound(ent, CHAN_AUTO, "tf2/alarm.wav", 1.0, ATTN_NORM, 0, PITCH_NORM)
...
public plugin_init() {
	...
	register_clcmd("centry_place", "createsentryhere")
	...
	g_menuId = register_menuid("\yРњРµРЅСЋ Р°РІС‚Рѕ-РїСѓР»РµРјС‘С‚РѕРІ")
...
public plugin_natives(){
	register_native("centry_place", "createsentryhere")
}
